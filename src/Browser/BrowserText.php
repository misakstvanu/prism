...
/**
 * The two rules every string out of a browser report has to meet before it can
 * become a column value (US-006).
 *
 ...
 * not the offending row, so one page posting a stray NUL would take the entire
 * batch it happened to travel in with it. Nothing between here and there would
 * report why: the batch is built on a queue, and the insert fails long after the
 * request that produced it has been answered 204.
 *
 * A page can post a NUL through perfectly valid JSON — the six characters that
 * spell the escape for code point zero decode to one — which is what makes
 * {@see clean} load-bearing rather than defensive. Invalid UTF-8 cannot survive
 * `json_decode` at all: it refuses a
 * malformed sequence and a lone surrogate escape — but it reaches this class by
 * the other door: the enrichment the host adds server-side (a `User-Agent`
 * header is a byte string an old client can put anything in) never passed
 * through a JSON parser.
 *
 * {@see truncate} is the byte-bounded half. Every cap in a browser payload is
